(ns sen.robustness.distance-metrics
...
(defn subopt-seq
  ...
  (let [n (*globals* :nsamples)
        cons-keys (third df)
        [_ substruct] (suboptimals neighbor n :centroid-only false)
        dist (fn [st1 st2]
               (if (zero? (count st1))
...
(defn remurna [f & {:keys [dir] :or {dir "/usr/local/remuRNA/"}}]
  (->> (shell/sh "./remuRNA" "-p=30" f :dir dir)
       :out
       str/split-lines
       (map #(str/split #"\t" %))
       rest
       (map #(vector (->> (first %) (re-find #"\d+") read-string);position
                     (->> % (take-last 2) first read-string)));H(wt||mut)
       (map second)
       (filter number?)
       mean ))
